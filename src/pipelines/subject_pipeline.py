...
class SubjectPipeline:
    """Pipeline for EMG-based user identification treating each subject as a single user
    
    This pipeline uses the SubjectLoader instead of UCIDataLoader, meaning:
    - Each subject (person) is treated as one biometric identity, regardless of arm
    - Both arms from the same subject will have the same biometric ID
    
    Otherwise, it follows the same steps as the UCIPipeline:
    ...
    def __init__(self, config_path: str = "config/uci_config.yaml", users_to_load: int = None, random_selection: bool = False):
        ...
        self.data_loader = SubjectLoader(config_path, users_to_load=users_to_load, random_selection=random_selection)
    ...
    def run(self, save_output: bool = True, train_model: bool = False, experiment_name: str = None) -> tuple:
        ...
        """
        print(f"\n{'='*50}")
        print(f"Starting EMG User Identification Pipeline (Subject-based)")
        print(f"{'='*50}")
